...
public:
	...
	//2020-12-15
	static int findTrayRect(cv::Mat srcm, cv::Rect &dstmrct,float lf_edge,float rt_edge, int tray_pix);
	static int _cutParcel_from_tray(const cv::Mat &srcm, cv::Mat &dstm, int tray_pix,int apply_rotate=0);

	//
	static int cutParcelMat(cv::Mat srcm, cv::Mat &parcel, float lf_edge, float rt_edge, \
		int tray_pix,int image_view = 0, int applid_rotate=0);//0:top,1:bottom,>1 side
	
	static int _findTrayRect_gray(const cv::Mat &srcm, cv::Rect &dstmrct, float lf_edge, float rt_edge, int tray_pix);

	static int _cutParcel_side(const cv::Mat &srcm, cv::Mat &dstm, int is_bottom=0);

	static int _cutParcel_bottom(const cv::Mat &srcm, cv::Mat &dstm, int apply_rotate=0);
