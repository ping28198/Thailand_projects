...
#define _TOSTRING(buf, fmt, val)	\
	...
	ccutil::vector<ccutil::RBBox> nmsR(vector<RBBox>& objs, float iou_threshold)
	{
		std::sort(objs.begin(), objs.end(), [](const RBBox& a, const RBBox& b) {
		...
		for (int i = 0; i < objs.size(); ++i) {
			...
			for (int k = i + 1; k < objs.size(); ++k) {
				if (flags[k] == 0) {
#ifdef IOU_ROTATE
					float iouUnion = objs[i].iouRtOf(objs[k]);
#else
	...
	void drawrbbox(cv::Mat& image, const RBBox& bbox, DrawType drawType, const string& labelText) {
		...
		rbox.center.x = bbox.x; rbox.center.y = bbox.y;
		rbox.size.width = bbox.h; rbox.size.height = bbox.w;
		rbox.angle = -bbox.a/3.1415926*180.;
		...
		for (int i = 0; i < 4; i++)
		{
			int lineThickness = 1;
			cv::Point pt1 = pts[i];
			cv::Point pt2 = (i==3)? pts[0]:pts[i+1];
			cv::line(image, pt1, pt2, color, lineThickness);
		}
		cv::putText(image, name, rbox.center, cv::FONT_HERSHEY_SIMPLEX, 1, color);
...
float ccutil::RBBox::width() const
{
	return this->w;
}

float ccutil::RBBox::height() const
{
	return this->h;
}

float ccutil::RBBox::area() const
{
	return width()*height();
}

cv::Point2f ccutil::RBBox::center() const
{
	return cv::Point2f(x, y);
}

float rough_dist(const float &w, const float&h)
{
	return std::max(fabs(w),fabs(h)) + std::min(fabs(w),fabs(h)) / 2.;
}


float ccutil::RBBox::iouRtOf(const RBBox& b) const
{
	auto &m = *this;
	cv::RotatedRect ra(m.center(), cv::Size2f(m.w, m.h), m.a);
	cv::RotatedRect rb(b.center(), cv::Size2f(b.w, b.h), b.a);

	// 粗略判断是否较远
	cv::Point2f distrect = m.center() - b.center();
	float dist = rough_dist(distrect.x, distrect.y);
	if ((rough_dist(m.width(), m.height()) + rough_dist(b.width(), b.height()))/2. < dist)
		return 0.0;

	std::vector<cv::Point2f> intcontour;
	int res = cv::rotatedRectangleIntersection(ra, rb, intcontour);
	if (res == cv::INTERSECT_NONE) return 0.;
	std::vector<cv::Point> contour;
	for (int i=0;i<intcontour.size();i++)
	{
		contour.push_back(intcontour[i]);
	}
	float intarea = cv::contourArea(contour);
	return intarea / (m.area() + b.area() - intarea + 1e-6);
}

float ccutil::RBBox::iouOf(const RBBox& b) const
{
	auto& a = *this;
	float xmax = max(a.x - a.w/2., b.x - b.w / 2.);
	float ymax = max(a.y - a.h / 2., b.y - b.h / 2.);
	float xmin = min(a.x + a.w / 2., b.x + b.w / 2.);
	float ymin = min(a.y + a.h / 2., b.y + b.h / 2.);
	float uw = (xmin - xmax + 1 > 0) ? (xmin - xmax + 1) : 0;
	float uh = (ymin - ymax + 1 > 0) ? (ymin - ymax + 1) : 0;
	float iou = uw * uh;
	return iou / (a.area() + b.area() - iou);
...
ccutil::RBBox::RBBox(float x, float y, float w, float h, float a, float score /*= 0*/, int label /*= 0*/):
	x(x),y(y),w(w),h(h),a(a),score(score),label(label)
{
