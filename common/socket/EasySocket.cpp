...
{	
    ...
    void DataSocket::Connect( ipaddress p_addr, port p_port )
    {
		...
		//std::cout << m_remoteinfo.sin_addr.s_addr << std::endl;
	...
	int ip_to_number(const std::string& _ip, ipaddress& i_ip)
	{
		using namespace std;
		istringstream iss;
		size_t        pos = 0;
		...
		for (int i = 0; i < 4; i++)
		{
			...
			//std::cout<<"ip "<<i<<":" << tmp << std::endl;
			i_ip += tmp;
			if (i!=3)
			{
				i_ip <<= 8;
			}
			//std::cout << hex << i_ip << std::endl;
		}

		return 1;
	}

	void DataSocket::Connect(const std::string &p_addr, port p_port)
	{
		//if (!ip_to_number(p_addr, addr)) return;
		//std::cout << addr<<std::endl;

		Connect(inet_addr(p_addr.c_str()), p_port);
	}

	void DataSocket::Bind(const std::string &p_addr, port p_port)
	{
		if (m_sock == -1)
		{
			m_sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
