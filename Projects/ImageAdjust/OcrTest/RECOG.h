...
#if !defined(RECOG_h)
#define RECOG_h
//#pragma comment(lib,"HW_SlantImage.lib")


#include <afx.h>
_declspec(dllimport) BYTE* HW_SlantImage(BYTE *, int &, int &);
_declspec(dllimport) int WINAPI LineAndWordSegment(BYTE *pBlockImage, int nWidth, int nHeight, char *lib_dir);
_declspec(dllimport) void FreeSlantImage();

//typedef BYTE*(__stdcall *HW_SlantImage)(BYTE *, int &, int &);
//typedef void(__stdcall *FreeSlantImage)();
//_declspec(dllimport) int WINAPI LineAndWordSegment(BYTE *pBlockImage, int nWidth, int nHeight, char *lib_dir);
