...
CString C_RECOG::GetRECOGRES()
{
	...
	BOOLFlag=SFile.Open(sFullPath_HWPrint+_T("Temp00.txt"),CFile::modeNoTruncate|CFile::modeRead|CFile::typeText);
	if (BOOLFlag==TRUE) 
	{
...
void C_RECOG::RECOG_HWPrint_Check(char *mPath)
{
	...
	sFullPath_HWPrint.Format(_T("%s"),mPath);	
	...
	try
	{
		//HW_SlantImage mSlantImage;
		//g_hDll = LoadLibrary(L"HW_SlantImage.dll");
		//if (g_hDll)
		//{
		//	mSlantImage = (HW_SlantImage)GetProcAddress(g_hDll, "HW_SlantImage");
		//	p = mSlantImage(pBIMG_HWPrint, nX, nY);
		//}
	...
	//FreeSlantImage freeSlantImage;
	//freeSlantImage = (FreeSlantImage)GetProcAddress(g_hDll, "FreeSlantImage");
	FreeSlantImage();
