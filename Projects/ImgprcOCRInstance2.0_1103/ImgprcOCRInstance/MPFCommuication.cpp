...
int  MPFCommuication::AnalyseOneComByte(BYTE byteData)
{
...
int MPFCommuication::AnalyseOneByte(BYTE byteData)
{
	unsigned int nMsgLen=0;
	unsigned int nBarNum=0;
	unsigned int i=0;
			...
			if (m_uchsRevMesNow[0] == MPF_MSG_ADDR_IADM	|| m_uchsRevMesNow[0] == MPF_MSG_ADDR_OCR )     //地址有效
			{
				...
				if(nMsgLen>0)
				{
					...
					if (check == m_uchsRevMesNow[nMsgLen])                 //LRC校验正确
					{
						...
						PushBackOneMessage(m_dwRevCount);
						//plogger->TraceInfo("确实推入了一个消息");
					}
				}
			}
		}
...
//在发送缓冲区中增加一个发送消息
int MPFCommuication::CodeMsgFromOCR2ImgServer(unsigned int nMsgLen, unsigned char * puchMsg, unsigned char* pBuffer)
{
	unsigned int newLen = 0;
	BYTE byte = 0;
	BYTE check = 0;
	BYTE uchNew[IADM_MES_MAX_LEN];
	...
	uchNew[newLen] = 0;
	strcpy((char*)pBuffer, (char*)uchNew);
	////将消息推入缓冲区
	//DWORD dwTemp1;
	//int bFlag = 0;
...
int MPFCommuication::PushBackOneMessage(DWORD _lenth)
{

	//m_dwRRevP;
	DWORD rlength = (_lenth < IADM_MES_MAX_LEN) ? _lenth : IADM_MES_MAX_LEN;
	memcpy(m_uchsRevMes[m_dwRRevP], m_uchsRevMesNow, rlength);
	m_dwRRevP++;
	m_dwRRevP = m_dwRRevP % IADM_REV_BUF_ROW;

	if (m_dwRRevP == m_dwRProcP)
	{
		m_dwRProcP++;
		m_dwRProcP = m_dwRProcP % IADM_REV_BUF_ROW;
	}
	return 1;
}
...
BYTE* MPFCommuication::GetOneMessage()
{
	if (m_dwRRevP == m_dwRProcP) return NULL;
	DWORD proP = m_dwRProcP;
	m_dwRProcP++;
	m_dwRProcP = m_dwRProcP % IADM_REV_BUF_ROW;
	return m_uchsRevMes[proP];
}

//成功1 失败0:接收无效 -1 缓冲区溢出 -2消息长度溢出
//在发送缓冲区中增加一个发送消息
int MPFCommuication::SendOneNetMsg(unsigned int nMsgLen, BYTE * puchMsg)
{
	unsigned int newLen  = 0;
	BYTE byte=0;
	BYTE check = 0;
